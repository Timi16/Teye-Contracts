...
use soroban_sdk::{
    ...
    Address, BytesN, Env, IntoVal, TryFromVal, Val, vec, Vec,
};
...
fn test_initialize_event() {
    ...
    let expected_topics: Vec<Val> = (symbol_short!("STREAM"), symbol_short!("ID_STAT")).into_val(&env);
...
fn test_guardian_changed_event() {
    ...
    let expected_topics: Vec<Val> = (symbol_short!("STREAM"), symbol_short!("ID_GUARD")).into_val(&env);
...
fn test_recovery_events() {
    ...
    let expected_topics_init: Vec<Val> = (symbol_short!("STREAM"), symbol_short!("ID_RINIT")).into_val(&env);
    ...
    let expected_topics_cncl: Vec<Val> = (symbol_short!("STREAM"), symbol_short!("ID_RCNCL")).into_val(&env);
    ...
    assert_eq!(exec_body.data, ScVal::try_from_val(&env, &expected_exec_val).unwrap());
    ...
    assert_eq!(deact_body.data, ScVal::try_from_val(&env, &expected_deact_val).unwrap());
    ...
    assert_eq!(act_body.data, ScVal::try_from_val(&env, &expected_act_val).unwrap());
}
...
fn test_zk_credential_event() {
    ...
    let expected_topics: Vec<Val> = (symbol_short!("STREAM"), symbol_short!("ID_ZKCRD")).into_val(&env);
